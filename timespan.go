...
// NewTimes creates a new span with the given start and end instants.
...
func (s Span) Encompass(r Span) Span {
	return Span{
		start: TimeMin(s.start, r.start),
		end:   TimeMax(s.end, r.end),
	}
}
...
func (s Span) Gap(r Span) Span {
	...
	return Span{
		start: TimeMin(s.end, r.end),
		end:   TimeMax(s.start, r.start),
	}
}
...
func (s Span) Intersection(r Span) (Span, bool) {
	...
	return Span{
		start: TimeMax(s.start, r.start),
		end:   TimeMin(s.end, r.end),
	}, true
}
...
// New(s.Start().AddDate(years, months, days), s.Duration()).
...
// TimeMax returns the later of two time instants.
func TimeMax(t, u time.Time) time.Time {
	if t.After(u) {
		return t
	}
	return u
...
// TimeMin returns the earlier of two time instants.
func TimeMin(t, u time.Time) time.Time {
	if t.Before(u) {
		return t
	}
	return u
